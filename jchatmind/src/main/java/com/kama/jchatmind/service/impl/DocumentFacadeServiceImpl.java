...
public class DocumentFacadeServiceImpl implements DocumentFacadeService {
    ...
    public CreateDocumentResponse uploadDocument(String kbId, MultipartFile file) {
        try {
            ...
            if ("mat".equalsIgnoreCase(filetype)) {
                metadata.setDocumentKind(VibrationAnalysisService.DOCUMENT_KIND_VIBRATION_MAT);
                metadata.setProcessingStatus(VibrationAnalysisService.PROCESSING_STATUS_PENDING);
            }
            ...
            } else if ("mat".equalsIgnoreCase(filetype)) {
                processMatDocument(documentId, filePath);
            } else {
    ...
    /**
     * 处理 MAT 振动文件，提取结构化元数据供工具使用
     */
    private void processMatDocument(String documentId, String filePath) {
        try {
            Document existingDocument = documentMapper.selectById(documentId);
            if (existingDocument == null) {
                log.warn("MAT 文档不存在，无法更新解析状态: {}", documentId);
                return;
            }
            ...
            metadata.setDocumentKind(VibrationAnalysisService.DOCUMENT_KIND_VIBRATION_MAT);
            metadata.setProcessingStatus(VibrationAnalysisService.PROCESSING_STATUS_READY);
            metadata.setParseError(null);
            metadata.setVibration(vibrationAnalysisService.inspectMatDocument(filePath));

            persistDocumentMetadata(existingDocument, documentDTO);
            log.info("MAT 文档解析完成: documentId={}", documentId);
        } catch (Exception e) {
            log.error("MAT 文档解析失败: documentId={}", documentId, e);
            markMatDocumentFailed(documentId, filePath, e.getMessage());
        }
    }

    private void markMatDocumentFailed(String documentId, String filePath, String parseError) {
        try {
            Document existingDocument = documentMapper.selectById(documentId);
            if (existingDocument == null) {
                return;
            ...
            metadata.setDocumentKind(VibrationAnalysisService.DOCUMENT_KIND_VIBRATION_MAT);
            metadata.setProcessingStatus(VibrationAnalysisService.PROCESSING_STATUS_FAILED);
            metadata.setParseError(parseError);
            metadata.setVibration(null);

            persistDocumentMetadata(existingDocument, documentDTO);
        } catch (Exception ex) {
            log.error("更新 MAT 文档失败状态时出错: documentId={}", documentId, ex);
        }
    }

    private void persistDocumentMetadata(Document existingDocument, DocumentDTO documentDTO) throws JsonProcessingException {
        Document updatedDocument = documentConverter.toEntity(documentDTO);
